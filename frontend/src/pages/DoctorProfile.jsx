...
export default function PatientProfile() {
  ...
  const fetchData = async () => {
    try {
      const apptRes = await axios.get(`http://localhost:5000/appointments/patient/${user.pid}`);
      setAppointments(apptRes.data?.appointments || []);

      const patientRes = await axios.get(`http://localhost:5000/patients/${user.pid}`);
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:5000/appointments/patients/${user.pid}`, formData);
